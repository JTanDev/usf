...
function onlyEvenValues(arr) {
  const evens = arr.filter(function (val) {
    return val % 2 == 0;
  });
  return evens;
}
...
function showFirstAndLast(arr) {
  const firstLast = arr.map(function (val) {
...
function valTimesIndex(arr) {
  const mulByIndex = arr.map(function (num, i) {
    return num * i;
  });
  return mulByIndex;
}
...
function find(arr, searchValue) {
  let search = arr.filter(function (val) {
...
function findInObj(arr, key, searchValue) {
  for (let obj of arr) {
    if (Object.values(obj).indexOf(searchValue) > -1) {
      return obj;
    }
  }
}
